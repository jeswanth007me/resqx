...
export const initialState = (): SimulationState => ({
  ...
  ambulance: {
    id: 'AMB-01',
    position: { x: 300, y: 40 },
    route: ['SIG-01', 'SIG-02', 'SIG-03', 'HOSPITAL'],
  ...
  decision: {
    action: 'NO_ACTION',
    signalId: null,
    reason: 'Waiting for emergency telemetry',
  },
...
export const tick = (
  ...
  const progress = Math.min(
    ...
    state.ambulance.progress +
      (elapsed * state.ambulance.speed) / 900
  );
  ...
  const road =
    state.roads.find(
      (item) =>
        item.id ===
        INITIAL_ROUTE[Math.min(2, Math.floor(progress * 3))]
    ) ?? state.roads[0];

  const position =
    progress >= 1
      ? HOSPITAL
      : pointOnRoad(road, (progress * 3) % 1);
  ...
  return {
    ...
    vehicles: state.vehicles.map((vehicle) => {
      const redSignal = state.signals.find(
        (item) =>
          item.road === vehicle.road &&
          item.state === 'RED'
      );

      const stoppedAtRed =
        redSignal &&
        vehicle.road === 'ROAD-02' &&
        vehicle.progress >= 0.45 &&
        vehicle.progress <= 0.58;

      return {
        ...vehicle,
        progress: stoppedAtRed
          ? vehicle.progress
          : (vehicle.progress +
              (elapsed * vehicle.speed) / 1600) %
            1,
      };
    }),
  };
